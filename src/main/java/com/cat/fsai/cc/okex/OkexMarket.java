...
import com.cat.fsai.type.tr.OkexTR;

@Service
public class OkexMarket implements MarketApi {

	private Logger logger = LoggerFactory.getLogger(this.getClass());


	final private String url = "https://www.okex.com/api/v1/";

	final String depth = "depth.do";
	...
	public void depth(DepthRes depthInfo, TR tr) {
		...
		try {
			...
			Optional<OkexTR> okexTR = OkexTR.searchByTr(tr);
			if (depthInfo == null) {
			...
			HttpGet request = new HttpGet(tagetUrl + "?symbol=" + okexTR.get().name() + "&size=20");
